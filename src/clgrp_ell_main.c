...
int main(int argc, char *argv[]) {
    ...
    if (argc != 5) {
        fprintf(stderr, "Format: mpirun -np [#procs] ./clgrp_ell [D_max] "
                        "[files] [ell] [folder]\n");
        fprintf(stderr, "\n");
    ...
    if (myrank == 0) {
        /* Coordinator: verify input files and distribute work */
        ...
        for (int ci = 0; ci < NUM_CONGRUENCES; ci++) {
            ...
            if (!verify_input_files_exist(folder, a, m, files)) {
                fprintf(stderr, "Error: not all input files exist for a=%d, m=%d\n", a, m);
                MPI_Abort(MPI_COMM_WORLD, 1);
            }
        }

        /* Verify D_max * ell^4 won't overflow long */
        {
            long ell2 = ell * ell;
            long ell4 = ell2 * ell2;
            if (ell2 / ell != ell || ell4 / ell2 != ell2 || D_max > LONG_MAX / ell4) {
                fprintf(stderr, "Error: D_max * ell^4 would overflow long.\n");
                MPI_Abort(MPI_COMM_WORLD, 1);
            }
        }
        ...
        for (int i = 0; i < active_workers; i++) {
            MPI_Recv(&finished[i], 1, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD,
                     MPI_STATUS_IGNORE);
        }
        for (int i = 0; i < active_workers; i++) {
            int term[3] = {-1, -1, -1};
        ...
        /* Ramare's bound */
        long h_max = (1 / M_PI) * sqrt(D_max) * (0.5 * log(D_max) + 2.5 - log(6)) + 1;
        h_max *= ell * (ell + 1);

        /* Smallest prime factor sieve */
        int *spf = (int *)malloc(h_max * sizeof(int));
        for (long j = 0; j < h_max; j++) {
            spf[j] = j;
        }
        for (long j = 2; j * j < h_max; j++) {
            if (spf[j] == j) {
                for (long k = j * j; k < h_max; k += j) {
                    if (spf[k] == k) {
                        spf[k] = j;
                    }
                }
        ...
        /* Worker: receive work items and process them */
